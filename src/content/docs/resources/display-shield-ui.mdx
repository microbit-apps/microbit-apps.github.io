...
shield to power both the shield and the micro:bit. You can also power the micro:bit and 
shield directly via USB plugged into the micro:bit or the display shield.
...
the screen are the A and B buttons (also referred to as `d-A` and
`d-B` to distinguish them from the micro:bit's A and B buttons).  

The D-pad is used to move  the cursor in the UI up/down/left/right. 
The `d-A` button is used to
select the current element the cursor is on. The `d-B` button is used
